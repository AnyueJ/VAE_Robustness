function  [OBJFUN  OBJGRAD] = OBJFUN_ECLIPSE(OPTPAR,NBLOCK,PARAM,PEQUIL,SWEQUIL,OBS,INJWELLSPEC,...
                                INJCONTROL,PRODWELLSPEC,PRODCONTROL,NT,NWINJ,NWPROD,CINV_OBS,...
                                CINV_PRIOR,OPTPAR_PRIOR,GAMMA)
...
switch PARAM
        case 'FULL'
            PERM = exp(OPTPAR);
        case 'REDUCED'
            LOGPERMFULL = PHI * OPTPAR + LOGPERM_MEAN;                     % approximated log-permeability field 
            LOGPERMFULL(LOGPERMFULL>16) = 16;
            LOGPERMFULL(LOGPERMFULL<0) = 0;
            PERM = exp(LOGPERMFULL);
end
...
CINV_OBS = ones(9,12);
CINV_OBS(1,:) = norm(OBS(1,:));
CINV_OBS(2:5,:) = norm(OBS(2:5,:));
CINV_OBS(6:9,:) = norm(OBS(6:9,:));

CINV_OBS = reshape(CINV_OBS,NOBS*NT,1);
...
PRIOR_TERM = (OPTPAR - OPTPAR_PRIOR)' * CINV_PRIOR * (OPTPAR- OPTPAR_PRIOR);
...
OBJGRAD_PRIOR = 2 * GAMMA * CINV_PRIOR * (OPTPAR - OPTPAR_PRIOR);
...
switch PARAM
    ...
    case 'REDUCED'                                                         % reduced permeability
        OBJGRAD = (OBJGRAD_OBS .* PERM)' * PHI  + OBJGRAD_PRIOR';
